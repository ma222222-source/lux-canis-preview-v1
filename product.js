...
function selectImage(url, label) {
  ...
  $$('[data-image]').forEach((button) => {
    const active = button.dataset.image === url;
...
$('#retry-image').addEventListener('click', () => selectImage(mainImage.getAttribute('src'), selectedColor));
...
function render() {
  ...
  const label = { new: '新作', low: '残りわずか', sold: '売り切れ' }[product.status] || '';
  ...
  const initialVariant = variants.find((variant) => variant.name === requestedColor) || variants[0];
  selectedColor = initialVariant?.name || '';
  $$('#color-options button').forEach((button) => button.setAttribute('aria-pressed', String(button.textContent === selectedColor)));
  selectImage(initialVariant?.image || images[0], selectedColor || 'メイン');
...
async function registerRestock() {
  const button = $('#restock-action');
  if (!button || button.disabled) return;
  button.disabled = true;
  button.textContent = '登録中…';
  $$('#color-options button').forEach((item) => { item.disabled = true; });
  try {
    ...
    if (response.status === 401) {
      ...
      next.searchParams.set('color', selectedColor);
      location.href = `./account.html?next=${encodeURIComponent(next.pathname + next.search)}`;
      return;
    }
    ...
    button.disabled = false; button.textContent = '再販待ちへ登録';
    $$('#color-options button').forEach((item) => { item.disabled = false; });
  }
}
